...
const DB_PATH = path.resolve(DATA_DIR, 'fabbytrack.db');
  ...
  prepare: (sql: string) => {
    ...
    return {
      run: (...args: unknown[]) => {
        ...
        return statement.run(...sanitized) as { changes: number; lastInsertRowid: number };
      },
      get: <T>(...args: unknown[]) => {
        const sanitized = args.map(arg => arg === undefined ? null : arg);
        return statement.get(...sanitized) as T | undefined;
      },
      all: <T>(...args: unknown[]) => {
        const sanitized = args.map(arg => arg === undefined ? null : arg);
        return statement.all(...sanitized) as T[];
      }
    };
  },
